...
func NewDefaultWriteHandler(session *internal.OpSession, req *pub.PrepareWriteRequest) (adapters.Writer, error) {
	...
	d.WriteSettings = &internal.WriteSettings{
		Schema:    req.Schema,
...
func (d *DefaultWriteHandler) Write(session *internal.OpSession, record *pub.UnmarshalledRecord) error {
...
func NewReplicationWriteHandler(session *internal.OpSession, req *pub.PrepareWriteRequest) (adapters.Writer, error) {
	...
	var settings internal.ReplicationSettings
...
order by id desc`, sqlSchema, constants.ReplicationVersioningTable, req.Schema.Id)
	...
	discoveredSchemas, err := session.SchemaDiscoverer.DiscoverSchemasSync(session, &pub.DiscoverSchemasRequest{
	...
	w.GoldenMetaSchema = internal.MetaSchemaFromPubSchema(goldenSchema)
	w.VersionMetaSchema = internal.MetaSchemaFromPubSchema(versionSchema)

	return w, nil
}

func (r *ReplicationWriter) Write(session *internal.OpSession, record *pub.UnmarshalledRecord) error {
...
func (r *ReplicationWriter) reconcileSchemas(session *internal.OpSession, current *pub.Schema, desired *pub.Schema) error {
...
func (r *ReplicationWriter) dropTable(session *internal.OpSession, table string) error {
...
func (r *ReplicationWriter) createTable(session *internal.OpSession, schema *pub.Schema) error {

	args := templates.ReplicationTableCreationDDL{
		Schema: internal.MetaSchemaFromPubSchema(schema),
	}
...
type NaveegoReplicationVersioningSettings struct {
	Changes       []string
	Settings      internal.ReplicationSettings
