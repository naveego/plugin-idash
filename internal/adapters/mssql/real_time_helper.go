...
func NewRealTimeHelper() (RealTimeHelper, error) {
	return RealTimeHelper{}, nil
}

func (r *RealTimeHelper) ensureDBChangeTrackingEnabled(session *internal.OpSession) error {

	if r.dbChangeTrackingEnabled {
		return nil
	}

	dbName := session.Settings.GetDatabase()
...
func (r RealTimeHelper) ensureTableChangeTrackingEnabled(session *internal.OpSession, schemaID string) error {
...
func (r RealTimeHelper) ConfigureRealTime(session *internal.OpSession, req *pub.ConfigureRealTimeRequest) (*pub.ConfigureRealTimeResponse, error) {
	var err error
	...
	if schemaInfo == nil || !schemaInfo.IsTable {
		schemaInfo = internal.MetaSchemaFromPubSchema(req.Schema)
	} else {
...
func commitVersion(session *internal.OpSession, out chan<- *pub.Record, version int) int {
	state := RealTimeState{
		Version: version,
	}
	r := &pub.Record{
...
func validateChangeTrackingVersion(session *internal.OpSession, schemaID string, version int) (bool, error) {
...
func getChangeTrackingVersion(session *internal.OpSession) (int, error) {
